...
function calc() {
  ...
  let persons = document.querySelectorAll('.counter-block-input')[0],
      ...
      daysSum = 0,
      total = 0;
  totalValue.innerHTML = 0; // или totalValue.textContent = 0;
...
function form() {
  ...
  form.addEventListener('submit', function (event) {
    //обработчик события на форме а не на кнопке отправки!
...
function slider() {
  ...
  function showSlides(n) {
    ...
    slides.forEach(item => item.style.display = 'none'); // for (let i = 0; i < slides.length; i++) { // старый стиль записи
  ...
  function plusSlides(n) {
    // увеличивает/уменьшает slideIndex в зависимости от того, в какуб сторону листаем слайды
    showSlides(slideIndex += n);
  }

  function currentSlide(n) {
    // для показа слайда при клике на соотв. точку
    showSlides(slideIndex = n);
  }
...
function timer() {
  // Timer
  let deadline = '2020-11-01'; // let deadline = '2020-10-10';
  ...
  function getTimeReminding(endtime) {
    ...
    let seconds = Math.floor(tmp / 1000 % 60),
        ...
        hours = Math.floor(tmp / (1000 * 60 * 60)); // hours = Math.floor((tmp / 1000 / 60 / 60) % 24),
...
__webpack_require__(/*! formdata-polyfill */ "../node_modules/formdata-polyfill/formdata.min.js"); // window.addEventListener('load'); // ждать полной загрузки страницы и только потом выполнять скрипт


window.addEventListener('DOMContentLoaded', function () {
  // ждать только загрузки структуры DOM
  'use strict';
